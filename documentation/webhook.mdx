...
In our system, an event is a specific state change in the lifecycle of a parcel that is monitored and notified via webhooks.
Each event represents a stage in the parcel's status.
When an event occurs, the webhook is triggered to inform external systems.
...
Events correspond to [Parcel Status Codes](/documentation/parcel_status_codes)
